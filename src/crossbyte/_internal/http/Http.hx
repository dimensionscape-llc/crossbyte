...
class Http
{
	...
	public function new(url:String, method:String = "GET", headers:Array<String> = null, requestData = null,
						contentType:Null<String> = null, data:Dynamic = null, version:HttpVersion = HttpVersion.HTTP_1_1, timeout:Int = 10000,
						userAgent:String = "XCrossByteHX", followRedirects:Bool = true)
	{
		__url = new URL(url);
		__headers = headers;
	...
	public function load():Void
	{
		...
		{
			while (__connected && (__status == 301 || __status == 302 || __status == 303 || __status == 307 && redirects.length < MAX_REDIRECTS))
			{
	...
	private function __handleResponse():Void
	{
		if (__connected)
		{
			...
			while (true)
			{
				...
				if (line == '') break; //end of response headers
