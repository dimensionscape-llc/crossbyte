...
class BitmapData
{
	...
	public function threshold(sourceBitmap:BitmapData, sourceRect:Rectangle, destPoint:Point, operation:String, threshold:Int, color:Int = 0, mask:Int = 0xFFFFFFFF, copySource:Bool = false):Int
	{
		var hits = 0;
		for (y in 0...Std.int(sourceRect.height))
		{
			for (x in 0...Std.int(sourceRect.width))
			{
				...
				switch (operation)
				{
					case "==": passed = (test == threshold); break;
					case "!=": passed = (test != threshold); break;
					case "<": passed = (test < threshold); break;
					case ">": passed = (test > threshold); break;
					case "<=": passed = (test <= threshold); break;
